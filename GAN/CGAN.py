...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.embedding = nn.Embedding(10, latent_embed_dim)
        self.model = nn.Sequential(
            nn.Linear(latent_dim+latent_embed_dim, 128),
            nn.BatchNorm1d(128),
    ...
    def forward(self, z, labels):
        ...
        output = self.model(torch.cat((z, embedding), dim=-1))
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.embedding = nn.Embedding(10, latent_embed_dim)
        self.model = nn.Sequential(
            nn.Linear((np.prod(image_size)+latent_embed_dim), 1024),
            nn.ReLU(inplace=True),
...
for i in range(num_epoch):
    for idx, data in enumerate(dataloader):
        ...
        g_loss = loss(discriminator(pred_images, labels), target)
        ...
        real_loss = loss(discriminator(gt_images, labels), target_real)
        fake_loss = loss(discriminator(pred_images.detach(), labels), target_f)
